...
function sumBy<T extends string | null>(rows: Row[], key: (r: Row) => T): Array<{ key: string; count: number; nameRo?: string | null; nameEn?: string | null }> {
...
export default function AudioStats() {
  ...
  useEffect(() => {
    ...
    window
      .fetch(`/api/stats/audio?range=${encodeURIComponent(range)}`, { signal: ctrl.signal })
      .then(async r => {
      ...
      .catch(() => {
        if (cancelled) return;
        setRows([]);
        setError(isRo ? 'Nu am putut încărca statisticile.' : 'Could not load stats.');
      })
      .finally(() => {
        if (cancelled) return;
        setLoading(false);
  ...
  const totals = useMemo(() => {
    const total = rows.reduce((acc, r) => acc + (Number.isFinite(r.accesses) ? r.accesses : 0), 0);
    const byCategory = sumBy(rows, r => r.categoryId);
    const byFlow = sumBy(rows, r => r.flowId);
    const byPractice = sumBy(rows, r => (r.flowId && r.practiceId ? `${r.flowId}__${r.practiceId}` : null));
      ...
      <IonContent fullscreen={true}>
        <div className="px-5 py-5 pb-10 w-full max-w-md md:max-w-2xl lg:max-w-3xl mx-auto flex flex-col gap-4">
          <div className="rounded-[16px] p-4 md:p-5" style={cardStyle}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(['24h', '7d', '30d', '365d'] as RangeKey[]).map(k => (
                <button
                  ...
                  style={{
                    backgroundColor: range === k ? 'rgba(197, 122, 74, 0.20)' : 'rgba(78, 91, 79, 0.06)',
                    color: '#4E5B4F',
                    border: range === k ? '1px solid rgba(197, 122, 74, 0.35)' : '1px solid rgba(232, 222, 211, 0.85)',
                  }}
                >
            ...
            <div className="mt-3 text-[13px]" style={{ color: '#7A746C', fontFamily: 'var(--font-logo), ui-serif, Georgia, serif' }}>
              {isRo ? 'Total accesări (estimare): ' : 'Total accesses (estimate): '}
              <span style={{ color: '#4E5B4F', fontWeight: 700 }}>{fmt(totals.total)}</span>
          ...
          {([
            { title: isRo ? 'Top categorii' : 'Top categories', items: totals.byCategory },
            { title: isRo ? 'Top flows' : 'Top flows', items: totals.byFlow },
            { title: isRo ? 'Top practici' : 'Top practices', items: totals.byPractice },
            ...
            <div key={section.title} className="rounded-[16px] p-4 md:p-5" style={cardStyle}>
              ...
              <div className="mt-3 flex flex-col gap-2">
                {section.items.slice(0, 20).map((it, idx) => (
                  <div key={it.key} className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-[12px] truncate" style={{ color: '#7A746C', fontFamily: 'var(--font-logo), ui-serif, Georgia, serif' }}>
                        {it.key}
                      </div>
                    </div>
                ...
                {section.items.length === 0 && !loading ? (
                  <div className="text-[13px]" style={{ color: '#7A746C', fontFamily: 'var(--font-logo), ui-serif, Georgia, serif' }}>
